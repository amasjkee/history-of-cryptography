      ...
      <div className="bg-card rounded-xl shadow-sm border border-border p-6">
        ...
        <div className="flex flex-wrap gap-4 mt-4">
          <button
            ...
            className="px-4 py-2 bg-secondary text-white rounded-lg hover:bg-secondary/80 transition-colors font-medium"
          >
            Очистить замены
          </button>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm text-muted-foreground">Уникальные замены</span>
          </label>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              className="w-4 h-4 accent-primary"
            />
            <span className="text-sm text-muted-foreground">Показать частоту</span>
          </label>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              className="w-4 h-4 accent-primary"
            />
            <span className="text-sm text-muted-foreground">Цифры и символы</span>
          </label>
        </div>
      </div>
      ...
      {ciphertext && (
        <>
          ...
          <div className="bg-card rounded-xl shadow-sm border border-border p-6">
            ...
            <div className="font-mono text-sm bg-secondary p-4 rounded-lg whitespace-pre-wrap break-words flex flex-wrap">
              {ciphertext.split('').map((char, i) => {
                ...
                return (
                  <div key={i} className="text-center mr-1 mb-1">
                    <div className="w-6 h-6 flex items-center justify-center bg-secondary rounded-t-md">{char}</div>
                    <div className={`w-6 h-6 flex items-center justify-center rounded-b-md ${isMapped ? 'bg-primary text-primary-foreground font-bold' : 'bg-secondary'}`}>
                      {decryptedChar}
                    </div>
                  </div>
                );
...
function MappingTile({ cipherChar, plainChar, onChange }: MappingTileProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="w-12 h-12 bg-secondary border-2 border-border rounded-lg flex items-center justify-center font-mono font-bold text-lg">
      ...
      <input
        ...
        maxLength={1}
        data-mapping-input
        className="w-12 h-12 text-center bg-background border-2 border-border rounded-lg focus:border-primary focus:ring-2 focus:ring-primary outline-none font-mono font-bold text-lg uppercase text-foreground"
      />
    </div>
  );
}